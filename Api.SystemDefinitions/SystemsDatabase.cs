...
namespace PolyhydraGames.Api.SystemDefinitions
{
    public class SystemsDatabase
    {
        ...
        {
            get => field ??
                   throw new InvalidOperationException("SystemsDatabase not initialized. Call Setup(ILogger) first.");
            private set;
        }

        public IReadOnlyList<SystemDefinition> Systems =>
            !_initialized ? throw new InvalidOperationException("Systems not initialized.") : _systems;
        ...
        public string ToFolder(string slug)
        {
        ...
        private SystemDefinition? FindByCore(string core)
        {
            return string.IsNullOrWhiteSpace(core)
                ? null
                : Systems.FirstOrDefault(x => string.Equals(x.Core, core, StringComparison.OrdinalIgnoreCase));
        }
